...
func (ab AthleticBrain) UpdateValue(b AthleticBrain, tal talent.Talents) {
	for key, val := range b {
		ab[key] += val * (100 + tal[talent.Athletic]) / 100
	}
}
...
func (mb MusicBrain) UpdateValue(b MusicBrain, tal talent.Talents) {
	for key, val := range b {
		mb[key] += val * (100 + tal[talent.Athletic]) / 100
	}
}
...
func (lb LanguageBrain) UpdateValue(b LanguageBrain, tal talent.Talents) {
	for key, val := range b {
		lb[key] += val * (100 + tal[talent.Athletic]) / 100
	}
}
...
func (mb MathBrain) UpdateValue(b MathBrain, tal talent.Talents) {
	for key, val := range b {
		mb[key] += val * (100 + tal[talent.Athletic]) / 100
	}
}
...
func (vb VisualBrain) UpdateValue(b VisualBrain, tal talent.Talents) {
	for key, val := range b {
		vb[key] += val * (100 + tal[talent.Athletic]) / 100
	}
}
