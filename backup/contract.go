...
func makeNewContract(aid, bid UID, term int) Contract {
	return Contract{
		AID:  aid,
		BID:  bid,
		ASig: false,
...
func (c *Contract) Sign(id UID) error {
	switch id {
	case c.AID:
		c.ASig = true
		return nil
...
type SoccerPlayerContract struct {
	...
	Buyout      float32 `json:"buyout"`
	// Bonus Clauses
	GoalBonus   float32 `json:"goal_bonus"`
	AssistBonus float32 `json:"assist_bonus"`
	EntryBonus  float32 `json:"entry_bonus"`
	// League Win Bonus
...
func MakeNewSoccerPlayerContract(
	a, b Payer,
	term int,
	salary, downPayment float32) *SoccerPlayerContract {
...
// GiveWage executes payment according to the contract.
func (sc *SoccerPlayerContract) GiveWage() error {
	// 1. Find character and team by ID
...
type Violation struct {
	ViolatorID    UID           `json:"aid"`
	CompensatorID UID           `json:"bid"`
